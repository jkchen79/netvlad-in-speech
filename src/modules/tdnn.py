...
class TDNN(nn.Module):
    ...
    def __init__(self, context, input_dim, output_dim, full_context=True, cuda=True):
        """
        ...
        self.kernel = nn.Parameter(
            torch.Tensor(output_dim, input_dim, self.kernel_width).normal_(0, stdv), requires_grad=True)
        self.bias = nn.Parameter(
            torch.Tensor(output_dim).normal_(0, stdv), requires_grad=True)
    ...
    def special_convolution(self, x, kernel, context, bias):
        ...
        xs = Variable(
            self.bias.data.new(batch_size, kernel.size()[0], len(valid_steps)))

        # Perform the convolution with relevant input frames
        for c, i in enumerate(valid_steps):
            x = x.contiguous()
            features = torch.index_select(
                x, 2, torch.add(Variable(context, requires_grad=False), i))
    ...
    def check_valid_context(context):
        ...
        assert context[0] <= context[
            -1], 'Input tensor dimensionality is incorrect. Should be a 3D tensor'
